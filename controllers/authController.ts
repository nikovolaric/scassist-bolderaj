...
export const loginReception = catchAsync(async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  if (!user.role.includes("employee") && !user.role.includes("admin")) {
    return next(
      new AppError("Nimaš dovoljenja, da se vpišeš v recepcijo.", 403)
    );
  }
...
export const loginAdmin = catchAsync(async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  if (!user.role.includes("admin")) {
    return next(
      new AppError("Nimaš dovoljenja, da se vpišeš v admin profil.", 403)
    );
  }
